...
export default function TopBar({line, nextStopStatus, closestStopIndex, RenderCurrentTime}){
    const status = nextStopStatus[closestStopIndex +1].replace("Abgefahren", "Endstation").replace("In Kürze erreichen wir", "Nächster Halt")
    const statuseng = nextStopStatus[closestStopIndex +1].replace("Abgefahren", "Final Stop").replace("In Kürze erreichen wir", "Next stop").replace("Nächster Halt", "Next Stop")
