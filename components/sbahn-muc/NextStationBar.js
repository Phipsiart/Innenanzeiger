export default function NextStationBar({ nextStop, nextStopStatus, linecolor }) {
  ...
  const statuseng = nextStopStatus
    .replace('Nächster Halt', 'Next stop')
    .replace('Abfahrt', 'Departure')
    .replace('Minuten ', 'min.')
  ...
  return (
    <>
      ...
      <div className="flex fixed top-2 left-5 z-30">
        <span className="text-[2.5rem] mt-14">
          <div>
        ...
        <span className="text-[2.5rem] fixed top-[4.2rem] left-[10.8rem]">
          {nextStop.arrivalDelay === 0 || nextStop.arrivalDelay === null
            ? '+0'
            : `+${Math.floor(nextStop.arrivalDelay / 60)}`}
        </span>
