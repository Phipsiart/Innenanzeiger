...
export async function Innenanzeiger(tripId) {
  ...
  try {
    const fetchdata = await fetch(`https://${API_INSTANCE}/trips/${encodeURIComponent(tripId)}`);
    ...
    const w9x0y1z2 = (o1p2q3r4 - s5t6u7v8) / 1000;

    // if the stop is 45 seconds or less away render the connections at the next stop
    if (w9x0y1z2 > 0 && w9x0y1z2 < 45) {
      renderconnections = true;
    } else if (w9x0y1z2 === 0) {
      renderconnections = false;
    }

    // for U-Bahn Munich, render the connections only if the stop is being approached and it is 30 seconds away or less
    if (data.trip.line.adminCode === 'swm001' && w9x0y1z2 <= 30) {
      renderconnections = true;
    }
    console.log(data);
    return {
    ...
    console.warn('Server error occcured.', error);
    return {
      servererror: true,
      message: 'A internal server occoured. Please try again later.',
    };
  }
}
